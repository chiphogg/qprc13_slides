...
SlidingVariables <- function(n_lines, n_points,
  ...
  i <- c(i1, i2)
  K.sliced <- K[i, i]
  L.sliced <- t(chol(K.sliced))
  seeds.sliced <- matrix(rnorm(n=2 * n_points), nrow=2)
  d_many_points <- data.frame(t(L.sliced %*% seeds.sliced))
  point_base_size <- 1
  ...
  p_scatter <- (ggplot(data=d_many_points)
    + geom_point(aes(x=X1, y=X2), size=point_base_size, colour='grey50')
    + geom_point(data=d_sampled_pts
      , aes(x=x1, y=x2, colour=as.factor(i))
...
i_mid <- round(N / 2)
di_min <- N * 0.05
fraction <- sin(pi * seq(from=1 / n_frames, to=1, length.out=n_frames)) ^ 2
di <- round(di_min + (N - i1 - di_min) * fraction)
saveGIF({
  for (i2 in i1 + di) {
    sv <- SlidingVariables(n_lines=12, n_points=200, x=X, i1=i1, i2=i2)
    grid.arrange(ncol=2
      , sv$lines
      , sv$scatter
      )
