...
PlotRandomFunctions <- function(i, n) {
  ...
  p <- (ggplot(data=d_old)
    + geom_line(data=d_master, aes(y=y, x=x, group=i), size=old.size*0.5, colour='grey70')
...
saveGIF({
  n <- 11
  for (i in 1:n) {
    grid.arrange(PlotRandomFunctions(i, n), PlotRandomFunctions(n, n), ncol=2)
  }
...
PlotAggregateUncertainty <- function(results, gap.size) {
  ...
  d_wide <- data.frame(gap=names(results),
    mean=unc_mean, max=unc_max, median=unc_median)
  require(reshape2)
  ...
  point_size <- 2
  require(ggplot2)
  p <- (ggplot(data=d, aes(x=as.numeric(gap), y=value))
    + geom_line(aes(colour=variable))
    + geom_point(aes(size=highlight), legend=FALSE)
    + scale_size_manual(values=c(no=point_size, yes=2 * point_size))
    + scale_x_continuous("Gap (mm)")
    + scale_y_log10()
    )
  return (p)
}
...
ani.width=150 * 4.0, ani.height=150 * 3.5)
```
...
```{r gap.size.uncertainty2, include=FALSE, cache=FALSE}
set.seed(1)
library(animation)
require(gridExtra)
saveGIF({
...
outdir=gif.output.directory,
clean=TRUE, interval=0.4, ani.dev='CairoPNG',
ani.width=150 * 7.5, ani.height=150 * 3.5)
```
