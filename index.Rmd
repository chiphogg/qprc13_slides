...
```{r make.side.by.side.gif, include=FALSE, cache=FALSE}
set.seed(1)
library(animation)
saveGIF({
  old.num <- 0
...
```{r many.side.by.side.function}
many_side_by_side <- function(k, n, i) {
  set.seed(2)
  X <- 1:k
  K <- 0.96 ^ abs(outer(X, X, "-"))
  L <- t(chol(K))
  ...
  p <- (ggplot(data=d_melt, aes(x=as.numeric(substring(variable, 3)), y=value, group=id, colour=highlight))
    ...
    + scale_x_continuous("variable", breaks=c(1, k), labels=c('a', 'b'))
    + scale_y_continuous("value", limits=c(-lim, lim))
...
```{r make.many.side.by.side.gif, include=FALSE, cache=FALSE}
set.seed(1)
library(animation)
saveGIF({
  k <- 15
...
```{r gap.size.error2, include=FALSE, cache=FALSE}
set.seed(1)
library(animation)
require(gridExtra)
saveGIF({
